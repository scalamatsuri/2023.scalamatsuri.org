...
const yaml_types_1 = require("./yaml_types");
admin.initializeApp();
 ...
 */
exports.storageHook = functions.storage.object().onFinalize(async (object, context) => {
    console.info('check object : %O', object);
    if (object.name === undefined) {
        console.warn('undefined object.name\n%O', object);
        return Promise.resolve();
    }
    const type = (0, yaml_types_1.selectYamlType)(object.name);
    if (type === yaml_types_1.YamlType.Unknown) {
        return Promise.resolve();
    }
    return createResultJson(type);
});
async function createResultJson(yamlType) {
    const getFilesResponse = await admin
        .storage()
        .bucket()
        .getFiles({
        directory: (0, yaml_types_1.srcFilesDirPath)(yamlType),
    });
    const distFilePath = (0, yaml_types_1.distResultPath)(yamlType);
